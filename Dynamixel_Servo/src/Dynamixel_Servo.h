...
#define SERVO_DEFAULT_BAUD               57142

#define SERVO_MAXIMUM_MOVING_SPEED       6.60   // rad / sec
//#define SERVO_MAXIMUM_MOVING_SPEED       5.76   // MX_28
 ...
 * @param       serial_port
 *                A pointer to the Arduino-defined serial port to which the motor is connected.
 *                &Serial, &Serial1, etc... Check the Arduino documentation for more details.
 *                You do not first need to call Serial.begin(), as this module will do that.
 * @param       half_duplex_direction_pin
 *                Arduino uses full-duplex serial communication (transmit and receive may
 *                happen simultaneously) whereas Dynamixel uses half-duplex (tx and rx must
 *                be sequential). Therefore you will have to connect your Dynamixel to your
 *                Arduino using the appropriate circuit: 
 *                http://support.robotis.com/en/product/dynamixel/dxl_mx_main.htm
 *                which uses one pin to control the direction (tx or rx) of data on the 
 *                bus. This module will control that pin, you just need to pass the 
 *                number of the Arduino digital io pin that you have connected to the circuit.
 * @param       baud
